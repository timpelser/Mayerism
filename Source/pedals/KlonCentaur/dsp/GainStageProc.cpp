...
GainStageProc::GainStageProc(double sampleRate)
    : preAmpL(sampleRate), preAmpR(sampleRate),
      clipL(sampleRate * os.getOversamplingFactor()),
      clipR(sampleRate * os.getOversamplingFactor()), ff2L(sampleRate),
      ff2R(sampleRate) {
  // No APVTS needed
}
