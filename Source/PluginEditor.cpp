...
NamJUCEAudioProcessorEditor::NamJUCEAudioProcessorEditor(
  ...
  resizeToggleAttachment.reset(
      new juce::AudioProcessorValueTreeState::ButtonAttachment(
          audioProcessor.apvts, "SMALL_WINDOW_ID", *hiddenResizeToggle));

  resizeButton->onClick = [this] {
    hiddenResizeToggle->setToggleState(!hiddenResizeToggle->getToggleState(),
                                       true);
    setPluginSize(
        *audioProcessor.apvts.getRawParameterValue("SMALL_WINDOW_ID"));
  };
...
void NamJUCEAudioProcessorEditor::setPluginSize(bool makeSmall) {
  if (makeSmall) {
    setResizeLimits(int(0.8 * 950) - 1, int(0.8 * 650) - 1, int(0.8 * 950),
                    int(0.8 * 650));
    namEditor.setScaleFactor(0.8);
    setSize(int(950 * 0.8), int(650 * 0.8));
    resizeButton->setBounds(getWidth() - 40, getHeight() - 70, 30, 30);
    resizeButton->setImages(false, true, false, arrowExpand, 0.35f,
                            juce::Colours::transparentWhite, arrowExpand, 0.6f,
                            juce::Colours::transparentWhite, arrowExpand, 1.0,
    ...
    resizeButton->setImages(false, true, false, arrowContract, 0.35f,
                            juce::Colours::transparentWhite, arrowContract,
                            0.6f, juce::Colours::transparentWhite,
                            arrowContract, 1.0, juce::Colours::transparentWhite,
                            0);
  }
}
